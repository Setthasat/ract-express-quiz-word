...
export class WordRepository {
    ...
    async deleteWord(user_id: string, word: string, part_of_speech: string): Promise<{ deletedWordName: string, deletedPartOfSpeech: string; } | null> {
        const user = await User.findOne({ user_id });
        if (!user) {
            console.log(`User with ID ${user_id} not found`);
            return null;
    ...
    async findExistWord(user_id: string, word: string, part_of_speech: string): Promise<IWord | null> {
        const user = await User.findOne({ user_id });
        if (!user) {
            console.log(`User with ID ${user_id} not found`);
            return null;
